                ...
                <div class="row d-flex justify-content-between" style="padding-right:15px;">
                    ...
                    <form action="" method="post" name="cartForm" id="cartForm">
                            ...
                            <script type="text/javascript">
								...
								function recipedel() {
									if (confirm("레시피를 삭제하시겠습니까?")){
										location.href="delrecipe?id=<%=rp.getR_id()%>";
									}
								}
							</script>
                            <%
                        ...
                        %>
	                        <script type="text/javascript">
	                        	function addCart() {
	                        		document.getElementById('cartForm').action = "addCart?name=<%=rp.getR_name() %>&len=<%=foodlen%>&file=<%=rp.getR_img() %>&id=<%=id%>";
	                        		document.cartForm.submit();
	                        	}
	                        	function goCart() {
	                        		document.getElementById('cartForm').action = "goCart?name=<%=rp.getR_name() %>&len=<%=foodlen%>&file=<%=rp.getR_img() %>";
	                        		document.cartForm.submit();
	                        	}
                        	</script>
                     	<%
                       	...
                       	<%
                        	...
                        	function addCart() {
                        		...
                        		document.getElementById('cartForm').action = "../login/login";
                        		document.cartForm.submit();
                        	}
                        	function goCart() {
                        		alert("로그인을 하신 후 이용해주세요.");
